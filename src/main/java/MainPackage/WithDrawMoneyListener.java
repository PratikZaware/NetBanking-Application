...
public class WithDrawMoneyListener implements ActionListener
{
    ...
    public WithDrawMoneyListener(Withdraw temp,String account_no)
    {
        ref=temp;
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        try
        {
            ...
            if(rs.first() && rs.getString("pass").equals(pass))
            {
                ...
                int amountToWithdraw=Integer.parseInt(ref.amountJTextField.getText());
                
                query="update bankcustomer set balance=balance-? where account_no=?;";
                pst=con.prepareStatement(query);
                pst.setInt(1,amountToWithdraw);
                pst.setString(2,accountno);
                
                pst.executeUpdate();
                JOptionPane.showMessageDialog(ref,"Amount withdrawn successfully!");
                Thread.sleep(5000);
                ref.dispose();
            }
            else
